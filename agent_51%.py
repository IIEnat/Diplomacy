...
class StudentAgent(Agent):
    ...
    def _base_values(self, utype):
        ...
        distinctions between SC/non‑SC and mine/neutral/enemy. A simple stage
        scalar (early/mid/late) boosts only **neutral** tiles:
          • Neutral SC and Neutral non‑SC multiplied by stage_scalar ∈ {1.5,1.25,1.0}
            (early=1.5, mid=1.25, late=1.0).
        """
        # Determine stage from phase year
        tag = self.game.get_current_phase() or ''
        try:
            year = int(tag[1:5])
        except Exception:
            year = None
        stage = _era_from_year(year)  # reuse helper for {early,mid,late}
        ...
        for n in G.nodes:
            ...
            if b in scs:
                if b in my_scs:
                    base = (1 + 1 * enemy_adj[n]) / neutral_mul        # My SC (no stage boost)
                else:
                    owner = None
                    for p in self.game.powers:
                        if b in self.game.get_centers(p):
                            owner = p; break
                    if owner is None:
                        base = 8 * neutral_mul           # Neutral SC (stage boost)
                    else:
                        base = 8.0 + (1 * sizes.get(owner,0))  # Enemy SC (no stage boost)
            else:
                if b in my_units:
                    base = 1                             # My non‑SC (no stage boost)
                elif b in enemy_units:
                    base = 3                             # Enemy non‑SC (no stage boost)
                else:
                    base = 3 * neutral_mul               # Neutral non‑SC (stage boost)
            vals[n] = float(base)
        return vals
    ...
    def get_actions(self):
        ...
        # 3) SPRING: recruit supports for attacks into enemy-held SCs
        if season == 'S':
            # Find our moves that target enemy-occupied SCs
            enemy_sc_attacks = []  # list of (attacker_loc, spear_order, dest_base)
            ...
            # For each such attack, try to convert nearby friendly units into supports
            used_supporters = set()
            for spear_loc, spear_order, dest_base in enemy_sc_attacks:
                ...
                for s_loc in orderable:
                    if s_loc == spear_loc or s_loc in used_supporters:
                        continue
                    poss_raw = all_poss.get(s_loc, [])  # use raw to allow support strings
